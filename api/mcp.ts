  ...
  server.setRequestHandler(CallToolRequestSchema, async (request, extra) => {
    ...
    try {
      switch (name) {
        ...
        case 'launch_agent': { const r: LaunchAgentRequest = { prompt: { text: a.prompt.text, images: a.prompt.images }, model: a.model, source: { repository: a.source.repository, ref: a.source.ref }, target: a.target, webhook: a.webhook }; return { content: [{ type: 'text', text: JSON.stringify(await apiClient.launchAgent(r), null, 2) }] }; }
        case 'add_followup': { const r: FollowUpRequest = { prompt: { text: a.prompt.text, images: a.prompt.images } }; return { content: [{ type: 'text', text: JSON.stringify(await apiClient.addFollowUp(a.id, r), null, 2) }] }; }
